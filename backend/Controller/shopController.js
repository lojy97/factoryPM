...
const ShopController = {
  ...
  createShop: async (req, res) => {
    try {
      ...
      if (!name || !logo || !email || !password) {
        return res
          .status(400)
          .json({ message: "Name, logo, email, and password are required" });
      }
  ...
  addProductToShop: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Product added to shop successfully", shop });
    } catch (error) {
      res.status(500).json({ message: "Server error", error: error.message });
    }
  },
  ...
  removeProductFromShop: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Product removed from shop successfully", shop });
    } catch (error) {
      res.status(500).json({ message: "Server error", error: error.message });
    }
  },
